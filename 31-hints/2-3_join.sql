...
--------------------------------
/*
ѕланы запросов - какие типы физических соединений таблиц помните?
«аставл€ют использовать 1 из 3х видов физических соединений дл€ конкретного соединени€
≈сть смысл использовать при соединении большого кол-ва таблиц, либо дл€ ad-hoc 

парсер окна сообщений
https://statisticsparser.com/
*/
-- ѕодготовка - отключим контроль прав доступа к Sales.Customers 
...
begin
	...
	-- несколько таблиц продажи клиентам с выводом покупател€ и плательщика
	...
	-----
	--ухудшение - зафиксировалс€ пор€док соединени€ (loop)
	select Client.CustomerName, Inv.InvoiceID, Inv.InvoiceDate, Item.StockItemName, Details.Quantity, Details.UnitPrice, PayClient.CustomerName as BillForCustomer
	from Sales.Invoices as Inv
	inner join Sales.InvoiceLines as Details on Inv.InvoiceID = Details.InvoiceID
	...
	--смотрим на кол-во чтений (если большое - надо разбиратьс€)
