...
FROM Warehouse.StockItems
 WINDOW W AS (ORDER BY UnitPrice);
...
SELECT UnitPrice, SupplierID, StockItemID, StockItemName, ColorId,
	ROW_NUMBER() OVER ord_price AS Rn,
	RANK() OVER ord_price AS Rnk,
	DENSE_RANK() OVER part_supplier AS DenseRnk,
	NTILE(4) OVER part_supplier AS GroupNumber
FROM Warehouse.StockItems
WHERE SupplierID in (5, 7)
 WINDOW ord_price AS  (ORDER BY UnitPrice),
        part_supplier AS (PARTITION BY SupplierId 
	                   ORDER BY UnitPrice)
ORDER By SupplierID, UnitPrice;
...
SELECT * 
FROM sales
ORDER BY NTILE(2) OVER (ORDER BY item_id);
...
SELECT sales_id, price_per_item,
  LAST_VALUE(price_per_item) IGNORE NULLS OVER( ORDER BY sales_id ) AS lastknowncol
FROM dbo.sales;
...
SELECT Invoices.InvoiceId, Invoices.InvoiceDate, Invoices.CustomerID, trans.TransactionAmount,
	...
	FROM Sales.CustomerTransactions as inr
		join Sales.Invoices as InvoicesInner ON 
			...
			AND InvoicesInner.CustomerID = inr.CustomerID
...
FROM Sales.Invoices as Invoices
	join Sales.CustomerTransactions as trans
		ON Invoices.InvoiceID = trans.InvoiceID
		AND Invoices.CustomerID = trans.CustomerID
WHERE Invoices.InvoiceDate < '2014-01-01'
ORDER BY Invoices.InvoiceId, Invoices.InvoiceDate;
...
SELECT Invoices.InvoiceId, Invoices.InvoiceDate, 
	...
	MAX(trans.TransactionAmount) OVER() AS MaxPayment
FROM Sales.Invoices as Invoices
	join Sales.CustomerTransactions as trans
		ON Invoices.InvoiceID = trans.InvoiceID
		AND Invoices.CustomerID = trans.CustomerID
...
SELECT Invoices.InvoiceId, Invoices.InvoiceDate, Invoices.CustomerID, trans.CustomerId, trans.TransactionAmount,
	...
	MAX(trans.TransactionAmount) OVER (),
...
WHERE Invoices.InvoiceDate < '2014-01-01'
and Invoices.CustomerID = 958
ORDER BY Invoices.InvoiceID--Invoices.CustomerID, trans.TransactionAmount ASC;
